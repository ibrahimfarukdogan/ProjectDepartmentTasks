...
import { Department, DepartmentTaskStatsResponse, DetailedDepartmentStatsResponse } from '@/types/departments';
import { LoginResponse, User, UserTaskStatsResponse } from '@/types/user';
...
export const createDepartment = async (dept_name: string, parent_id: number | null): Promise<Department | null> => {
  try {
    const response = await API.post(`api/departments`, { dept_name, parent_id });
...
export const getDepartment = async (departmentId: number): Promise<Department | null> => {
  try {
...
export const createUserInDepartment = async (departmentId: number, user: User | null): Promise<User | null> => {
  try {
